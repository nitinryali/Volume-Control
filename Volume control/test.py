...
while (cap.isOpened()):
    ...
    if results.multi_hand_landmarks:
        ...
        if len(lmList) != 0:
            ...
            cv2.circle(img, (x1, y1), 15, (0, 110, 255), cv2.FILLED)
            cv2.circle(img, (x2, y2), 15, (0, 110, 255), cv2.FILLED)
            cv2.line(img, (x1, y1), (x2, y2), (0, 110, 255), 3, cv2.FILLED)
            ...
            cv2.rectangle(img, (50, 150), (85, 400), (255, 255, 255))
            cv2.rectangle(img, (50, int(volBar)), (85, 400), (255, 255, 255), cv2.FILLED)
            cv2.putText(img, f'{int(volPer)} %', (85, 450), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0,110,255 ), 3)
            vc.SetMasterVolumeLevel(vol, None)

    CTime = time.time()
    fps = 1 / (CTime - PTime)
    PTime = CTime
    cv2.putText(img, str(int(fps)), (40, 50), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0, 110, 255), 3)
